...
export class ConditionLab extends FormApplication {
	constructor(object, options = {}) {
		...
		this.initialMapType = game.settings.get("flavien-condition-lab", "conditionMapType");
		this.mapType = null;
		this.initialMap = game.settings.get("flavien-condition-lab", "activeConditionMap");
		this.map = null;
		this.displayedMap = null;
		this.maps = game.settings.get("flavien-condition-lab", "defaultConditionMaps");
		this.filterValue = "";
		this.sortDirection = "";
	}
	...
	async getData() {
		...
		const defaultMaps = game.settings.get("flavien-condition-lab", "defaultConditionMaps");
		// const mappedSystems = Object.keys(defaultMaps) || [];
		const mappedSystems = [];
		const mapTypeChoices = game.settings.settings.get("flavien-condition-lab.conditionMapType").choices;
		...
		const triggers = game.settings.get("flavien-condition-lab", "storedTriggers").map((t) => {
			return [t.id, t.text];
		});
	...
	_processFormData(formData) {
		...
		let newMap = [];
		const rows = [];
		const existingMap = this.map ?? game.settings.get("flavien-condition-lab", "activeConditionMap");
	...
	async _restoreDefaults({ clearCache = false } = {}) {
		...
		let defaultMaps = game.settings.get("flavien-condition-lab", "defaultConditionMaps");

		if (clearCache) {
			defaultMaps = await EnhancedConditions._loadDefaultMaps();
			game.settings.set("flavien-condition-lab", "defaultConditionMaps", defaultMaps);
		}
	...
	async _updateObject(event, formData) {
		const showDialogSetting = game.settings.get("flavien-condition-lab", "showSortDirectionDialog");
		...
		if (this.sortDirection && showDialogSetting) {
			await Dialog.confirm({
				...
				yes: ($html) => {
					...
					if (checkbox.checked) {
						game.settings.set("flavien-condition-lab", "showSortDirectionDialog", false);
					}
	...
	async _saveMapping(newMap, mapType = this.mapType) {
		...
		await game.settings.set("flavien-condition-lab", "conditionMapType", mapType);
		await game.settings.set("flavien-condition-lab", "activeConditionMap", preparedMap);
	...
	async _importFromJSONDialog() {
		new Dialog({
			...
			content: await renderTemplate("modules/flavien-condition-lab/templates/import-conditions.html", {}),
			buttons: {
				import: {
	...
	async _onClickActiveEffectConfig(event) {
		...
		const conditions = this.map ?? game.settings.get("flavien-condition-lab", "activeConditionMap");
		...
		if (!foundry.utils.hasProperty(conditionEffect, "flags.flavien-condition-lab.conditionId")) {
			setProperty(
				conditionEffect,
				"flags.flavien-condition-lab.conditionId",
				conditionId
			);
		}
