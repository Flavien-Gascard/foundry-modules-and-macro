...
export class EnhancedConditions {
	...
	static _processActiveEffectChange(effect, type = "create") {
		...
		const conditionId = effect.getFlag("flavien-condition-lab", "conditionId");
		...
		const toOutput = conditions.filter((condition) => (isDefault && game.settings.get("flavien-condition-lab", "defaultConditionsOutputToChat"))
			|| (game.settings.get("flavien-condition-lab", "conditionsOutputToChat") && condition?.options?.outputChat));
		...
		if (!type.active && enhancedConditionsDiv && sameSpeaker && recentTimestamp) {
			...
			for (const condition of conditions) {
				...
				const newRow = await renderTemplate(
					"modules/flavien-condition-lab/templates/partials/chat-card-condition-list.hbs",
			...
			const content = await renderTemplate(
				"modules/flavien-condition-lab/templates/chat-conditions.hbs",
				templateData
			);
	...
	static async _loadDefaultMaps() {
		const path = "modules/flavien-condition-lab/condition-maps";
	...
	static _prepareMap(conditionMap) {
		...
		const outputChatSetting = game.settings.get("flavien-condition-lab", "conditionsOutputToChat");
	...
	static getConditionsMap() {
		let conditions = game.settings.get("flavien-condition-lab", "activeConditionMap");
		if (!game.settings.get("flavien-condition-lab", "removeDefaultEffects")) {
	...
	static _updateStatusEffects(conditionMap) {
		const removeDefaultEffects = game.settings.get("flavien-condition-lab", "removeDefaultEffects");
		const activeConditionMap = conditionMap || game.settings.get("flavien-condition-lab", "activeConditionMap");
	...
	static _prepareActiveEffects(effects) {
		...
		for (const effect of effects) {
			const overlay = foundry.utils.getProperty(effect, "flags.flavien-condition-lab.core.overlay");
	...
	static getConditionsByIcon(icon) {
		const conditionMap = game.settings.get("flavien-condition-lab", "activeConditionMap");
	...
	static getDefaultMap(defaultMaps = null) {
		...
		defaultMaps =
			defaultMaps instanceof Object
				? defaultMaps
				: game.settings.get("flavien-condition-lab", "defaultConditionMaps");
			...
			if (hasDuplicates) {
				...
				*/
				...
				for (const effect of effects) {
					...
					const conditionId = foundry.utils.getProperty(effect, `flags.flavien-condition-lab.${"conditionId"}`);
					const matchedConditionEffects = existingConditionEffects.filter(
						(e) => e.getFlag("flavien-condition-lab", "conditionId") === conditionId
					);
	...
	static getConditions(entities = null, { warn = true } = {}) {
		...
		const map = game.settings.get("flavien-condition-lab", "activeConditionMap");
		...
		for (let entity of entities) {
			...
			const effectIds =
				effects instanceof Array
					? effects.map((e) => e.getFlag("flavien-condition-lab", "conditionId"))
					: effects.getFlag("flavien-condition-lab", "conditionId");
	...
	static getConditionEffects(entities, map = null, { warn = true } = {}) {
		...
		if (!map) map = game.settings.get("flavien-condition-lab", "activeConditionMap");
		...
		for (const entity of entities) {
			...
			const conditionEffects = activeEffects.filter((ae) => ae.getFlag("flavien-condition-lab", "conditionId"));
		...
		for (let entity of entities) {
			...
			const conditionEffect = actor.effects.contents.some((ae) => {
				return conditions.some(
					(e) =>
						e?.flags["flavien-condition-lab"].conditionId
						=== ae.getFlag("flavien-condition-lab", "conditionId")
				);
			});
