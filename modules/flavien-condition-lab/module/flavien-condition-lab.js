...
Hooks.on("i18nInit", () => {
	...
	// Keybinds
	game.keybindings.register("flavien-condition-lab", "openConditionLab", {
	...
	game.keybindings.register("flavien-condition-lab", "openTriggler", {
	...
	if (!game.modules.get("status-halo")?.active && !game.modules.get("illandril-token-hud-scale")?.active) {
		...
		libWrapper.register(
			...
			function () {
				const effectSize = game.settings.get("flavien-condition-lab", "effectSize");
...
Hooks.on("ready", async () => {
	...
	let conditionMap = game.settings.get("flavien-condition-lab", "activeConditionMap");

	const mapType = game.settings.get("flavien-condition-lab", "conditionMapType");
	...
	if (!mapType && defaultMaps instanceof Object && Object.keys(defaultMaps).includes(game.system.id)) {
		game.settings.set("flavien-condition-lab", "conditionMapType", "default");
	}
	...
	if (!conditionMap.length) {
		...
		if (game.user.isGM && conditionMap.length) {
			game.settings.set("flavien-condition-lab", "activeConditionMap", conditionMap);
		}
	}
	...
	// const specialStatusEffectMap = game.settings.get("flavien-condition-lab", "specialStatusEffectMapping");
...
Hooks.on("updateCombat", (combat, update, options, userId) => {
	const enableOutputCombat = game.settings.get("flavien-condition-lab", "conditionsOutputDuringCombat");
...
Hooks.on("getSceneControlButtons", function (hudButtons) {
	if (game.user.isGM && game.settings.get("flavien-condition-lab", "sceneControls")) {
...
Hooks.on("renderSettingsConfig", (app, html, data, ...others) => {
	const trigglerMenu = $(html).find("button[data-key=\"flavien-condition-lab.trigglerMenu\"]")[0];
	if (trigglerMenu) {
...
Hooks.on("renderMacroConfig", (app, html, data) => {
	...
	const flag = app.object.getFlag("flavien-condition-lab", "macroTrigger");
	const triggers = game.settings.get("flavien-condition-lab", "storedTriggers");

	const select = foundry.applications.fields.createSelectInput({
		name: "flags.flavien-condition-lab.macroTrigger",
		options: triggers,
		value: flag,
