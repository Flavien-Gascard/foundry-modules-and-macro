...
export class TrigglerForm extends FormApplication {
	...
	activateListeners(html) {
		...
		deleteTrigger.on("click", async (event) => {
			...
			await game.settings.set("flavien-condition-lab", "storedTriggers", updatedTriggers);
			this.data = {};
			this.render();
		});
	...
	async _updateObject(event, formData) {
		...
		const setting = await game.settings.set("flavien-condition-lab", "storedTriggers", updatedTriggers);
	 ...
	 */
	_exportToJSON() {
		const triggers = foundry.utils.duplicate(game.settings.get("flavien-condition-lab", "storedTriggers"));
		const data = {
	...
	async _processImport(html) {
		...
		const originalTriggers = game.settings.get("flavien-condition-lab", "storedTriggers");
		await game.settings.set("flavien-condition-lab", "storedTriggers", originalTriggers.concat(triggers));
		this.render();
	}
